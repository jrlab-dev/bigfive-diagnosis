...
// しかし10問版ではgetRarityが'r0'固定なので、隠しキャラは表示されない
{
  r = calcScore('10', [1,5,1,5,1,5,1,5,1,5], questions10);
  assert(r.code === '11111', '10問版: 全低回答(+1/-5) → コード11111（ゾロと同コードだがr0固定）', `実際: ${r.code}`);
}
...
// MAX因子カウントでレア度判定
function getRarityFromCode(code, version) {
  if (version === '10') return 'r0';

  // MAX因子カウント
  let maxCount = 0;
  if (isMax('O', parseInt(code[0]))) maxCount++;
  if (isMax('C', parseInt(code[1]))) maxCount++;
  if (isMax('E', parseInt(code[2]))) maxCount++;
  if (isMax('A', parseInt(code[3]))) maxCount++;
  if (isMax('N', parseInt(code[4]))) maxCount++;
...
{
  const code = '33333';
  const rarity = getRarityFromCode(code, '30');
  assert(rarity === 'r1', `コード${code}(MAX 0個) → R1`, `実際: ${rarity}`);
}
...
// R2: MAX 1個 (O=5)
{
  const code = '53333';
  const rarity = getRarityFromCode(code, '30');
  assert(rarity === 'r2', `コード${code}(MAX 1個:O) → R2`, `実際: ${rarity}`);
}
...
// R2: MAX 1個 (E=1)
{
  const code = '33133';
  const rarity = getRarityFromCode(code, '30');
  assert(rarity === 'r2', `コード${code}(MAX 1個:E) → R2`, `実際: ${rarity}`);
}
...
// R2: MAX 1個 (N=1)
{
  const code = '33331';
  const rarity = getRarityFromCode(code, '30');
  assert(rarity === 'r2', `コード${code}(MAX 1個:N) → R2`, `実際: ${rarity}`);
}
...
// R3: MAX 2個 (O=5,C=5)
{
  const code = '55333';
  const rarity = getRarityFromCode(code, '30');
  assert(rarity === 'r3', `コード${code}(MAX 2個:O,C) → R3`, `実際: ${rarity}`);
}

// R4: MAX 3個 (O=5,C=5,E=5)
{
  const code = '55533';
  const rarity = getRarityFromCode(code, '30');
  assert(rarity === 'r4', `コード${code}(MAX 3個:O,C,E) → R4`, `実際: ${rarity}`);
}

// R5: MAX 4個 (O=5,C=5,E=5,A=5,N=1)
{
  const code = '55551';
  const rarity = getRarityFromCode(code, '30');
  assert(rarity === 'r5', `コード${code}(MAX 4個:O,C,E,A,N) → R5`, `実際: ${rarity}`);
}
...
// R5: MAX 5個 (全MAX)
{
  const code = '55511';
  const rarity = getRarityFromCode(code, '30');
  assert(rarity === 'r5', `コード${code}(MAX 5個) → R5`, `実際: ${rarity}`);
}

// 10問版はr0固定
{
  const code = '55555';
  const rarity = getRarityFromCode(code, '10');
  assert(rarity === 'r0', `10問版コード${code} → R0`, `実際: ${rarity}`);
}
